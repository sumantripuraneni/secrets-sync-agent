...
def discover():
    ...
    oauth_server_info = requests.get(url, verify=ca_cert)
...
def validate_existing_token(token):
    ...
    try:
        if ocp_access_token is not None:
            ...
            ocp_healthz_query=requests.get(ocp_healthz_url, verify=ca_cert, headers=ocp_healthz_headers)
        else:
...
def get_access_token(token=None):
    ...
    ocp_custom_creds = get_secret(vault_url=vault_url,secret_path=secret_path,k8_hvault_token=k8_hvault_token)

    if ocp_custom_creds is None:
        log.info("Credentials to work with OCP cluster are not found in Vault, path "+ secret_path)
    ...
    if not token_valid:
        ...
        log.info("Existing token is not defined or doesn't work, need to regenerate a new one")

        oauth_server_info=discover()

        log.info("Using following endpoint to get the token - " + oauth_server_info['token_endpoint'])
        ...
        authorization_url, state = openshift_oauth.authorization_url(
            oauth_server_info["authorization_endpoint"], state="1", code_challenge_method="s256"
        )
        ...
        qwargs = {
            k: v[0]
            for k, v in parse_qs(urlparse(challenge_response.headers["Location"]).query).items()
        }
        ...
        log.info("Authorization token to work with OpenShift cluster was successfully obtained")    
